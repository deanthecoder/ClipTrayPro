...
/// <summary>
/// Periodically records process and managed-heap details when memory usage crosses configured thresholds.
/// </summary>
/// <remarks>
/// Comparing private bytes with managed heap sizes helps distinguish retained .NET objects from native clipboard,
/// rendering, or operating-system allocations without perturbing the process with a forced collection.
/// </remarks>
public sealed class MemoryMonitor : IDisposable
{
    ...
    public MemoryMonitor(int thresholdMegabytes, Func<string> appDetails)
    {
        ...
        ThresholdMegabytes = thresholdMegabytes;
    }

    public int ThresholdMegabytes
    {
        get => checked((int)(m_thresholdBytes / BytesPerMegabyte));
        set
        {
            m_thresholdBytes = Math.Max(1, value) * BytesPerMegabyte;
    ...
    public void Start()
    {
        ...
        log.Info($"Memory monitor enabled: samples every {SampleInterval.TotalSeconds:N0} seconds, " +
                 $"reports every {ThresholdMegabytes:N0} MB. Log: {log.File.FullName}");
    ...
    private MemorySnapshot CaptureSnapshot()
    {
        ...
        return new MemorySnapshot(
            ...
            m_appDetails?.Invoke() ?? "Unavailable");
    }
    ...
    private static void LogSnapshot(string reason, MemorySnapshot snapshot, bool isWarning)
    {
        var report = new StringBuilder(reason)
            ...
            .AppendLine()
            .Append("ClipTrayPro: ").Append(snapshot.AppDetails);
